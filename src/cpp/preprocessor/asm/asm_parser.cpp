  ...
  while (std::getline(isstr, line)) {
    ...
    if (operate_directive(line))
    {
      ...
      for (int count = 0 ; count < 3 ; ++count)
      {
        ...
        aline = trim(aline);
        if (!aline.substr(0,3).compare("id:"))
          id_line = trim(aline.substr(aline.find(":") + 1));
        else if (!aline.substr(0,5).compare("name:"))
          name_line = trim(aline.substr(aline.find(":") + 1));
        else if (!aline.substr(0,12).compare("description:"))
          description_line = trim(aline.substr(aline.find(":") + 1));
        else
