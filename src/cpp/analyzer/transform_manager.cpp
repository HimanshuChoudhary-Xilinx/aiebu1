...
process_sections() {
  ...
  for (const auto& section_ptr : m_elfio.sections) {
    ...
    if (is_text_section(section_name) && section->get_type() == ELFIO::SHT_PROGBITS) {
      ...
      bool is_merged = (m_elfio.sections[get_ctrldata_section_name(col, page, id)] == nullptr);
      if (is_merged)
...
get_controlcode_bd_offset(const std::string& section_name, uint32_t offset, symbol::patch_schema schema)
{
  ...
  if (!ctrltext)
    throw error(error::error_code::internal_error, "ctrltext section for col:"
  ...
  if (ctrldata) {
    // Legacy per-page format: offset = T_N + D_bd, ctrltext size = 16 + T_N.
    // Adjust to get D_bd as the index into ctrldata.
    ...
    // Merged single-section format: offset is the absolute byte position of
    // the BD within the merged ctrltext section.
    if (offset >= ctrltext->get_size())
...
set_controlcode_bd_offset(const std::string& section_name, uint32_t offset, uint64_t bd_offset, symbol::patch_schema schema)
{
  ...
  if (ctrldata) {
    ...
    // Merged single-section format: offset is the absolute byte position in ctrltext
    if (offset >= ctrltext->get_size())
      throw error(error::error_code::internal_error, "merged ctrltext offset out of range:"
                  + std::to_string(offset) + "\n");
